...
class Get_data_model extends CI_Model {
	...
	public function reupdatestatustime($prid,$datetimeupdate,$statusappval)
	{
		if ($this->session->username=='Somkhit') {
			$userby = 'GM';
		}elseif ($this->session->username=='Nalinee') {
		...
		if ($statusappval!='') {
		$data = array(
			'rc_statusdatetime' => $datetimeupdate,
			'rc_statusby' => $userby);
		}else{
		$data = array(
			'rc_statusdatetime' => NULL,
			'rc_statusby' => '');
		}
		$this->db->where('prno', $prid);
		$this->db->update('PR_ref', $data);
	...
	public function approveY($primary,$approvedata,$type,$date,$username)
	{
		...
		if ($type=='approval' AND $username=='Nalinee' OR $type=='approval' AND $username=='Somkidc') {
			$PR_ref = array(
	...
	public function AC_appovecheck()
	{
		...
		$this->db->join('PR_ref', 'PR_ref.prno = PR.prno');
		//$this->db->join('Coss_PR', 'Coss_PR.prno = PR.prno');
		$where = "((dep = 'AC') AND (GMApprove is null) AND (EFCApprove is null) AND (PRApprove is null) AND (completed is null) AND (warecode != '' ) OR  (dep != 'AC') AND (GMApprove is null) AND (EFCApprove is null) AND (PRApprove is null) AND (completed is null) AND (warecode != '' ) AND (HdApprove = 'Y'))";
		$this->db->where($where);
		$this->db->order_by("PR_ref.prno", "desc");
		$result = $this->db->get()->result_array();
		return $result;
	...
	public function delte_pr_is_null()
	{
		...
		$this->db->from('PR');
		$this->db->join('PR_ref', 'PR_ref.prno = PR.prno');
		$this->db->where('prdate !=', $date);
		$this->db->where('Vendor IS NULL',null, false);
		$this->db->where('Vendor_name IS NULL',null, false);
		$result = $this->db->get()->result_array();
		foreach ($result as $row) {
			$this->db->where('prno', $row['prno']);
			$this->db->from('PR_Item');
			$item_null = $this->db->count_all_results();
			if ($item_null == '0') {
				$this->db->delete('PR', array('prno' => $row['prno']));
				$this->db->delete('PR_ref', array('prno' => $row['prno']));
				$this->db->delete('PR_Item', array('prno' => $row['prno']));
			}
		}
	}
