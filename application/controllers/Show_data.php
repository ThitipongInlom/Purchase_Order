...
class Show_data extends CI_Controller {
	...
	public function show_completed()
	{
		...
		$datesearchstart = $this->input->get('datesearchstart');
		$datesearchend = $this->input->get('datesearchend');
		$searchpraa = $this->input->get('searchpraa');
		if (empty($datesearchstart) AND empty($datesearchend)) {
			$dateforstart = 'N';
			$dateforend = 'N';
		}else{
		...
		if (empty($searchpraa)) {
			...
			$searchpraa = $searchpraa;
		}
		$set = $this->Switch_model->show_completed();
		if ($set == '0') {
		$data['row'] = $this->Get_data_model->Get_completed($dateforstart,$dateforend,$searchpraa);
		$this->load->view('theme/head');
	...
	public function show_completed2()
	{
		$checkdate = date('Y-m-d');
		$datesearchstart = $this->input->get('datesearchstart');
		$datesearchend = $this->input->get('datesearchend');
		...
		$set = $this->Switch_model->show_completed();
		if ($set == '0') {
		$data['row'] = $this->Get_data_model->Get_completed2($dateforstart,$dateforend);
		$this->load->view('theme/head');
		$this->load->view('show_completed2',$data);
		$this->load->view('theme/footer');
		}else{
		$this->load->view('theme/head');
		$this->load->view('modifly');
	...
	public function acappovecheck()
	{
		$data['row'] = $this->Get_data_model->AC_appovecheck();
		$this->load->view('theme/head');
		$this->load->view('acapprove',$data);
		$this->load->view('theme/footer');
	}
	public function acapproveaction()
	{
		$selected = $_POST['selected'];
		foreach ($selected as $key => $value) {
			$this->Get_data_model->acapproveaction_model($value);
		}
		echo 'OK';
	}
	public function modal_opendata()
	{
		...
		function Getimgshowpr($prno)
		{
			...
			$query = $beta->get_where('PR_img', array('prno' => $prno));
			$result = $query->result_array();
			return $result;
		}
		...
		$Data = Getimgshowpr($data_head[0]['prno']);
		if (!empty($Data)) {
			echo '<div class="col-md-9 col-xs-9"><b>Img PR:</b> ';
			$i = 1;
			foreach ($Data as $key => $rowimg) {
				echo '  <span class="label label-primary" imgdata="'.$rowimg['part_img'].'" onclick="openwindowimg(this)" style="cursor: pointer;"><i class="fa fa-paperclip">  รูปภาพที่ '.$i.' </i></span>';
			$i++;
			}
			echo '</div><div class="col-md-3 col-xs-3"></div>';
		}
		echo '<br>
		<div class="col-md-12 col-xs-12">
			<div class="table-responsive">
				<table  width="100%" class="table table-bordered table-condensed  table-striped">
					<tr style="background:#D5DBDB;">
					  <th width="10%" rowspan="2" style="text-align: center; vertical-align: middle;"><b>No.</b></th>
				...
				echo'
					...
					if ($data_head[0]['DC']>0) {
						...
						echo '<tr style="background:#D5DBDB;">
							<td colspan="9" style="text-align: right;"><b>Discount '.$dispersen.'%</b></td>
					...
					if ($data_head[0]['DC_A']>0) {
						echo '<tr style="background:#D5DBDB;">
							<td colspan="9" style="text-align: right;"><b>Discount</b></td>
							<td>';$DV_V = $data_head[0]['DC_A'];  echo number_format($DV_V,2).''; echo'</td>
						</tr>';
						}
					if ($data_head[0]['Vat']=='Y') {
						echo '<tr style="background:#D5DBDB;">
							<td colspan="9" style="text-align: right;"><b>Vat 7%</b></td>
							<td>';
								if ($data_head[0]['DC_A']>0) {
									$totalget = $this->Get_data_model->gettotal($data_head[0]['prno']);
									$total_discount_c = $data_head[0]['DC_A'];
								}else{
									$total_discount_c = "0";
								}
								if ($data_head[0]['DC'] > 0) {
									$total_discount_a = ($data_head[0]['DC'] * $totalget)/100;
								}else{
									$total_discount_a = "0";
								}
					...
					if ($data_head[0]['Vat']=='Y' OR $data_head[0]['DC_A']>0 OR $data_head[0]['DC']>0) {
						echo '<tr style="background:#D5DBDB;">
							...
							$gtotal = $totalget - $DV_V + $vata; echo number_format($gtotal,2).'';  echo'</td>
						</tr>';
					}
				echo'</table>';
	...
	if ($data_head[0]['statusapp'] !='') {
		echo '<br>';
		echo '<b>Status PR:</b>  '.$data_head[0]['statusapp'];
		echo '<br>';
	}
	echo '<br>';
	...
	$user = $this->session->username;
        ...
        echo '<button type="button" class="btn btn-success" onclick="approve(this)" data-toggle="tooltip" data-placement="bottom" title="อนุมันติPR">อนุมันติPR</button>
			...
			if ($data_head[0]['chkre']=='' AND $this->session->dep =='AC' AND $this->session->type =='accounting' OR $data_head[0]['chkre']=='' AND $this->session->dep =='AC' AND $this->session->type =='accounting0' OR $data_head[0]['chkre']=='' AND $this->session->dep =='AC' AND $this->session->username =='dang' OR  $data_head[0]['chkre']=='' AND $this->session->dep =='AC' AND $this->session->username =='soda' ) {
	...
	public function resetstatusapp()
	{
		$prid = $_POST['prid'];
		$deppr = $_POST['deppr'];
		$statusappval = $_POST['statusappval'];
		...
		$this->Get_data_model->restatusapp($prid,$statusappval,$userby);
		$this->Get_data_model->reupdatestatustime($prid,$datetimeupdate,$statusappval);
		if ($statusappval!='') {
			$this->bot_line_setting($statusappval,$prid,$userby,$datetimeupdate,$deppr);
		}
	}
	...
	public function approveX_sd()
	{
		...
		$username = $this->session->username;
		$date = date('YmdHi');
		$this->Get_data_model->approveX_sd($primary,$approvedata,$type,$date,$username);
	}

	public function approveXold()
	{
		$primary = $_POST['primary'];
		$approvedata = 'N';
		$type = $this->session->type;
		$username = $this->session->username;
		$date = date('YmdHi');
		$this->Get_data_model->approveXold($primary,$approvedata,$type,$date,$username);
	}

	public function hold_pr()
	{
		$primary = $_POST['primary'];
		$date = date('YmdHi');
		$this->Get_data_model->hold_pr($primary,$date);
	}
	...
	public function completedY_No_Po()
	{
		$primary = $_POST['primary'];
		$approvedata = $_POST['approvedata'];
		$this->Get_data_model->completedY($primary,$approvedata);
	...
	{
		$this->Get_data_model->Vendor004completedY_AC($prno);
	}
	...
	public function completedY_AC()
	{
		$primary = $_POST['primary'];
		$approvedata = $_POST['approvedata'];
		$this->Vendor004completedY_AC($primary);
		//$refno_pr = $this->Get_data_model->Get_refno_pr($primary);
		//$set_app = $this->Get_data_model->Get_Zign_Beta($refno_pr);
		if ($set_app == '0') {
			//$this->Add_Pr_BO($primary);
